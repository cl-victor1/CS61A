...
class VendingMachine:
    ...
    def vend(self):
        if self.total == 0:
            ...
            print(f"'Here is your {self.product} and ${self.funds - self.price} change.'")
            self.funds = 0
            self.total -= 1
    ...
    def restock(self, num):
        self.total += num
        return f"Current {self.product} stock: {self.total}"
...
def store_digits(n):
    ...
    while n > 10:
        n = n // 10   
        tens += 1
    return Link(n, store_digits(original - n * 10 ** tens))
...
def two_list(vals, counts):
    ...
    if counts[0] >= 1:        
        ...
        return Link(first, two_list(vals[1:], counts[1:]))
