...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def select(string):
        for target in string.lower().split():
            for word in subject:
                if remove_punctuation(target) == word:
                    return True
        return False
    return select
    # END PROBLEM 2
...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    times = []
    for person in timestamps_per_player:
        individual_time = [person[i + 1] - person[i] for i in range(len(person) - 1)]
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match, j, i) < current_time:
                current_time = time(match, j, i)
                fast_person = j
        res[fast_person].append(get_word(match, i))
    return res
    # END PROBLEM 10
